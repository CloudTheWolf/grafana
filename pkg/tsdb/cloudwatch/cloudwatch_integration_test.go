...
func Test_CloudWatch_CallResource_Integration_Test(t *testing.T) {
	...
	t.Run("Should handle dimension value request and return values from the api", func(t *testing.T) {
		...
		executor := newExecutor(im, &fakeSessionCache{}, featuremgmt.WithFeatures())
	...
	t.Run("Should handle region requests and return regions from the api", func(t *testing.T) {
		im := defaultTestInstanceManager()
		executor := newExecutor(im, &fakeSessionCache{}, featuremgmt.WithFeatures(featuremgmt.FlagCloudwatchNewRegionsHandler, true))
		req := &backend.CallResourceRequest{
			Method: "GET",
			Path:   `/regions`,
			PluginContext: backend.PluginContext{
	...
	t.Run("Should handle legacy region requests and feature toggle is turned off", func(t *testing.T) {
		im := defaultTestInstanceManager()
		executor := newExecutor(im, &fakeSessionCache{}, featuremgmt.WithFeatures(featuremgmt.FlagCloudwatchNewRegionsHandler, false))
		req := &backend.CallResourceRequest{
			Method: "GET",
			Path:   `/regions`,
			PluginContext: backend.PluginContext{
	...
	t.Run("Should error for any request when a default region is not selected", func(t *testing.T) {
		...
		executor := newExecutor(imWithoutDefaultRegion, &fakeSessionCache{}, featuremgmt.WithFeatures(featuremgmt.FlagCloudwatchNewRegionsHandler, false))
		req := &backend.CallResourceRequest{
			Method: "GET",
			Path:   `/regions`,
			PluginContext: backend.PluginContext{
